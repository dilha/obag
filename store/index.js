...
export const mutationTypes = {
  ...
  loadSubcategorySuccess:
    'mutation/SubcategorySuccess load subcategory success',
...
export const actions = {
  [actionTypes.loadNews]({ commit }) {
    return new Promise((resolve) => {
      this.$api.get('/page/get-news').then((response) => {
  ...
  [actionTypes.loadCategory]({ commit }, id) {
    return new Promise((resolve) => {
      this.$api.get(`/category/${id}`).then((response) => {
  ...
  [actionTypes.loadSubcategory]({ commit }, id) {
    return new Promise((resolve) => {
      this.$api.get(`/subcategory/${id}`).then((response) => {
...
export const getters = {
  likesProducts: (state) =>
    state.category?.products?.length ? state.category.products.slice(0, 4) : [],
